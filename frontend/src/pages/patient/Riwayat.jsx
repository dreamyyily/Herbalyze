    ...
    >
      ...
      >
        <div className="p-6">
          ...
          <h2 className="text-xl font-black text-slate-800 leading-snug mb-1">{primaryName}</h2>
          ...
          <div className="flex items-center gap-2.5 p-3 bg-rose-50 rounded-xl border border-rose-100 mb-5">
            ...
            <p className="text-[11px] font-black text-rose-700 uppercase tracking-tight">{herb.reason}</p>
          </div>
          ...
          <div className="space-y-3">
            ...
            <div className="bg-slate-50 rounded-2xl p-4 border border-slate-100">
              <h4 className="text-[10px] font-black text-slate-400 uppercase tracking-[0.18em] mb-2 flex items-center gap-1.5">
                <Info size={12} className="text-indigo-500" /> Analisis Efek Samping
              </h4>
              <p className="text-[13px] text-slate-600 leading-relaxed">{herb.description}</p>
            </div>
            ...
            {herb.reference && herb.reference !== "-" && (
              <div className="bg-indigo-50/60 rounded-2xl p-4 border border-indigo-100/60">
                <h4 className="text-[10px] font-black text-slate-400 uppercase tracking-[0.18em] mb-2 flex items-center gap-1.5">
                  <BookOpen size={12} className="text-indigo-600" /> Referensi Klinis
                </h4>
                <p className="text-[11px] text-indigo-800/80 italic leading-relaxed">"{herb.reference}"</p>
              </div>
            )}
          </div>
          ...
          <div className="p-5 sm:p-6 space-y-5">
            ...
            {showMedicalWarning && (
              <div className="bg-danger-30/10 border-l-4 border-danger-30 p-4 rounded-r-2xl shadow-sm animate-[fadeIn_0.5s_ease-out]">
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    <h4 className="text-sm font-bold text-danger-30 mb-1">Instruksi Keamanan Medis!</h4>
            ...
            {aliasNames && (
              <div className="bg-light-20 border border-light-40 rounded-2xl p-4">
                ...
                <p className="text-dark-40 text-sm leading-relaxed font-medium">{aliasNames}</p>
              </div>
            )}
            ...
            <div>
              ...
              <div className="flex flex-col gap-4">
                {partsArray.map((partName, idx) => {
                  ...
                  if (isSocfindo) {
                    ...
                    renderPrep = clean.length > 1
                      ...
                      : <div className="bg-light-10/80 p-4 rounded-xl border border-light-40/50">
                          <p className="text-dark-40 text-sm leading-relaxed">{clean[0]?.endsWith(".") ? clean[0] : (clean[0] || "") + "."}</p>
                        </div>;
                  } else {
                    ...
                    renderPrep = (
                      <div className="space-y-3">
                        {paragraphs.map((para, pIdx) => {
                          ...
                          return (
                            <div key={pIdx} className="bg-light-10/80 p-4 rounded-xl border border-light-40/50">
                              ...
                              <p className="text-dark-40 text-sm leading-relaxed text-justify">{content.replace(/[\r\n]+/g, " ")}</p>
                            </div>
                          );
                        })}
                      </div>
                  ...
                  return (
                    <div key={idx} className={`${isMultiPart ? "bg-primary-10/30 border-primary-20" : "bg-light-20/60 border-light-40"} border rounded-2xl p-4 sm:p-5 hover:shadow-sm transition-all`}>
            ...
            <div className="pt-4 border-t border-dashed border-light-40">
              <h4 className="text-xs font-bold text-dark-30 uppercase tracking-widest mb-3">Sumber Kajian &amp; Referensi</h4>
              <div className="flex flex-col gap-2">
                {sourceLinks.length === 0 && sourceLabels.length === 0 && (
                  <span className="text-sm text-dark-30 italic">Tidak ada referensi tertaut.</span>
                )}
                ...
                {sourceLabels.map((label, i) => {
                  ...
                  return (
                    <div key={`label-${i}`} className="inline-flex items-start gap-3 p-3 rounded-xl bg-light-20 text-sm text-dark-40 border border-light-40">
                      <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5 flex-shrink-0 text-dark-30 mt-0.5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
...
// ─── Card Riwayat ─────────────────────────────────────────────────────────────
function RiwayatCard({ hist, index, onDelete, onSelectHerb, isFavorite, onToggleFavorite, isSelectMode, isSelected, onToggleSelect }) {
  ...
  const totalUnsafe  = hist.recommendations?.reduce((s, g) => s + (g.unsafe_herbs?.length || 0), 0) || 0;
  ...
  return (
    <div className="animate-[staggeredFadeIn_0.5s_ease-out_forwards]" style={{ opacity: 0, animationDelay: `${index * 80}ms` }}>
      {selectedUnsafe && <UnsafeHerbModal herb={selectedUnsafe} onClose={() => setSelectedUnsafe(null)} />}
      ...
      >
        <div className="p-6 md:p-8">
          ...
          <div className="flex items-center justify-between mb-3">
            ...
            {isSelectMode && (
              <div
                onClick={(e) => { e.stopPropagation(); onToggleSelect(hist.id); }}
          ...
          <div className="flex items-start justify-between gap-4 mb-4">
            <h3 className={`text-lg md:text-xl font-extrabold leading-snug transition-colors ${isSelected ? "text-red-600" : "text-gray-800"}`}>
          ...
          {hasCondition && (
            <div className="flex flex-wrap gap-2 mb-4">
              {hist.special_conditions.filter(c => c !== "Tidak ada").map((c, i) => (
                <span key={i} className="inline-flex items-center gap-1 px-3 py-1 bg-amber-50 border border-amber-200 text-amber-700 text-xs font-semibold rounded-full">⚠️ {c}</span>
              ))}
            </div>
          )}

          <div className="flex flex-wrap items-center gap-2 mb-5">
            <span className={`inline-flex items-center gap-1.5 px-3 py-1.5 text-xs font-bold rounded-full ${totalHerbs > 0 ? "bg-primary-10 text-primary-50" : "bg-gray-50 border border-gray-100 text-gray-400"}`}>
              🌿 {totalHerbs} Herbal Direkomendasikan
            </span>
            {totalUnsafe > 0 && (
              <span className="inline-flex items-center gap-1.5 px-3 py-1.5 bg-rose-50 text-rose-600 text-xs font-bold rounded-full border border-rose-100">⛔ {totalUnsafe} Perlu Dihindari</span>
            )}
            {hasChemical && (
              <span className="inline-flex items-center gap-1.5 px-3 py-1.5 bg-orange-50 text-orange-600 text-xs font-bold rounded-full border border-orange-100">💊 Obat Kimia</span>
            )}
            {totalGroups > 0 && (
              <span className="inline-flex items-center px-3 py-1.5 bg-gray-50 border border-gray-100 text-gray-500 text-xs font-semibold rounded-full">{totalGroups} kategori</span>
            )}
          </div>
            ...
            <div className="flex items-center justify-between pt-4 border-t border-gray-100">
              {hasAnyResult ? (
                ...
                <div className="inline-flex items-center gap-2 px-4 py-2.5 bg-amber-50 border border-amber-100 rounded-full">
                  ...
                  <span className="text-xs font-bold text-amber-700">Tidak ada herbal yang relevan ditemukan</span>
                </div>
              )}
              <button
          ...
          {isSelectMode && (
            <div className="pt-4 border-t border-gray-100">
              <p className="text-xs text-center font-semibold text-gray-400">
                {isSelected ? "✅ Dipilih untuk dihapus" : "Ketuk card untuk memilih"}
              </p>
            </div>
          )}
        </div>
              ...
              return (
                <div key={gi} className="space-y-5">
                  <div className="flex items-center gap-2">
                    <div className={`w-9 h-9 rounded-xl flex items-center justify-center shrink-0 ${isDiagnosis ? "bg-indigo-100 text-indigo-600" : "bg-orange-100 text-orange-600"}`}>
                      {isDiagnosis ? (
                        <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                    ...
                    <div className="flex-1 min-w-0">
                      <p className={`text-[10px] font-extrabold uppercase tracking-widest ${isDiagnosis ? "text-indigo-500" : "text-orange-500"}`}>{group.group_type}</p>
                      <p className="text-sm font-extrabold text-gray-800 capitalize leading-tight">{group.group_name?.replace("Terkait: ", "")}</p>
                    </div>
                    {groupHasHerbs && <span className="shrink-0 text-xs text-gray-400 font-semibold">{group.herbs.length} herbal</span>}
                  </div>
                  ...
                  {group.mapping_info && (
                    <div className="flex items-start gap-3 bg-indigo-50/70 border border-indigo-100 p-4 rounded-2xl">
                      ...
                      <p className="text-xs text-indigo-700 italic leading-relaxed">{group.mapping_info}</p>
                    </div>
                  )}
                  ...
                  {groupHasHerbs && (
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                      {group.herbs.map((herb, hi) => (
                        <div key={hi} onClick={() => onSelectHerb(herb, hasChemical)}
                          ...
                          <div className="w-20 shrink-0 bg-gradient-to-b from-primary-10/50 to-green-50 flex items-center justify-center overflow-hidden">
                            {herb.image ? (
                              <img src={herb.image} alt={herb.name} className="w-full h-full object-cover group-hover/herb:scale-105 transition-transform duration-300"
                                onError={(e) => { e.target.onerror = null; e.target.parentElement.innerHTML = '<span style="font-size:1.6rem">🌿</span>'; }} />
                          ...
                          <div className="flex-1 p-3.5 min-w-0">
                            {(() => {
                              ...
                              return <p className="text-sm font-extrabold text-gray-800 leading-snug line-clamp-1 group-hover/herb:text-primary-50 transition-colors mb-0.5" title={clean}>{primary}</p>;
                            })()}
                            {herb.latin && <p className="text-xs text-gray-400 italic mb-1.5 truncate">{herb.latin}</p>}
                            {herb.part && (
                              <span className="inline-flex items-center gap-1 text-[10px] bg-green-50 text-green-700 border border-green-100 px-2 py-0.5 rounded-full font-semibold">🍃 {herb.part}</span>
                            )}
                  ...
                  {groupHasUnsafe && (
                    <div>
                      <div className="flex items-center justify-between mb-4">
                        <div>
                          <h4 className="text-sm font-black uppercase tracking-[0.2em] text-rose-600">⛔ Herbal yang Perlu Dihindari</h4>
                          <p className="text-sm text-slate-500 mt-1">Sistem mendeteksi {group.unsafe_herbs.length} herbal yang berisiko untuk kategori ini.</p>
                        </div>
                        <span className="px-3 py-1.5 bg-rose-50 text-rose-700 border border-rose-200 rounded-full text-xs font-bold">{group.unsafe_herbs.length} Risiko</span>
                      </div>
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        {group.unsafe_herbs.map((unsafe, ui) => (
                          <div key={ui} className="bg-white border border-rose-100 rounded-[24px] p-5 flex flex-col justify-between shadow-sm hover:border-rose-300 transition-all">
                            <div>
                              ...
                              <h5 className="text-base font-black text-slate-800 mb-2">{unsafe.name}</h5>
                            ...
                            <button
                              onClick={(e) => { e.stopPropagation(); setSelectedUnsafe(unsafe); }}
...
function DeleteModal({ hist, multiCount, onConfirm, onCancel, isDeleting }) {
  ...
  const allInputs = !isMulti ? [...(hist?.diagnoses || []), ...(hist?.symptoms || [])] : [];
  return (
      ...
      <div className="bg-white rounded-[28px] shadow-2xl max-w-sm w-full p-8 animate-[slideUp_0.25s_ease-out]">
        ...
        {isMulti ? (
          <p className="text-gray-400 text-sm text-center mb-6 leading-relaxed">
            <span className="font-bold text-red-500">{multiCount} riwayat</span> yang dipilih akan dihapus secara permanen.
          </p>
        ) : (
          <>
        ...
        <div className="flex gap-3">
          <button onClick={onCancel} disabled={isDeleting} className="flex-1 px-4 py-3 bg-gray-100 hover:bg-gray-200 text-gray-700 rounded-2xl font-bold transition-colors text-sm">Batal</button>
          <button onClick={onConfirm} disabled={isDeleting} className={`flex-1 px-4 py-3 rounded-2xl font-bold text-white text-sm transition-all bg-red-500 hover:bg-red-600 ${isDeleting ? "opacity-50 cursor-not-allowed" : ""}`}>
            {isDeleting ? "Menghapus..." : isMulti ? `Hapus ${multiCount} Riwayat` : "Ya, Hapus"}
          </button>
        </div>
      </div>
    </div>
...
function startOfLocalCalendarDay(d) { return new Date(d.getFullYear(), d.getMonth(), d.getDate()); }
...
export default function Riwayat() {
  ...
  const fetchHistory = useCallback(async () => {
    ...
    if (!wallet) { setError("Sesi Anda belum aktif. Silakan masuk terlebih dahulu."); setIsLoading(false); return; }
    try {
      const res = await fetch(`${API}/api/history/${wallet}`);
      if (!res.ok) throw new Error("Gagal memuat data.");
      setHistories(await res.json());
    } catch { setError("Koneksi terputus. Pastikan server sedang berjalan."); }
  ...
  const handleToggleFavorite = useCallback((histId) => {
    setFavorites(prev => { const next = new Set(prev); next.has(histId) ? next.delete(histId) : next.add(histId); saveFavorites(next); return next; });
  }, []);
  ...
  const filteredHistories = histories.filter((hist) => {
    ...
    if (timeFilter === "minggu") matchesTime = matchesPeriodFromToday(hist.created_at, 7);
    else if (timeFilter === "bulan") matchesTime = matchesPeriodFromToday(hist.created_at, 30);
    return matchesSearch && matchesTime;
  });

  const isAllSelected = filteredHistories.length > 0 && filteredHistories.every(h => selectedIds.has(h.id));
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      setFavorites(prev => { const next = new Set(prev); next.delete(deleteTarget.id); saveFavorites(next); return next; });
  ...
  const handleMultiDelete = async () => {
    ...
    let successCount = 0; const failedIds = [];
    await Promise.all(idsToDelete.map(async (id) => {
      try { const res = await fetch(`${API}/api/history/${id}?wallet_address=${wallet}`, { method: "DELETE" }); if (!res.ok) throw new Error(); successCount++; }
    ...
    setFavorites(prev => { const next = new Set(prev); deletedIds.forEach(id => next.delete(id)); saveFavorites(next); return next; });
    setIsDeleting(false); setShowMultiDeleteModal(false); exitSelectMode();
    failedIds.length === 0
      ? setNotification({ type: "success", message: `${successCount} riwayat berhasil dihapus.` })
      : setNotification({ type: "error", message: `${successCount} berhasil, ${failedIds.length} gagal dihapus.` });
  };

  const totalHerbsAll  = histories.reduce((s, h) => s + (h.recommendations?.reduce((x, g) => x + g.herbs.length, 0) || 0), 0);
  const totalFavorites = histories.filter(h => favorites.has(h.id)).length;
  const isFavoritTab   = timeFilter === "favorit";

  return (
    <MainLayout>
      {deleteTarget && <DeleteModal hist={deleteTarget} multiCount={1} onConfirm={handleDelete} onCancel={() => setDeleteTarget(null)} isDeleting={isDeleting} />}
      {showMultiDeleteModal && <DeleteModal hist={null} multiCount={selectedIds.size} onConfirm={handleMultiDelete} onCancel={() => setShowMultiDeleteModal(false)} isDeleting={isDeleting} />}
      ...
      <div className="max-w-2xl mx-auto pt-14 md:pt-20 px-4 pb-32 min-h-[75vh]">
        ...
        <div className="text-center mb-12 animate-[fadeIn_0.5s_ease-out]">
          ...
          <p className="text-gray-500 text-base max-w-sm mx-auto font-medium leading-relaxed">Semua hasil analisis herbal Anda tersimpan di sini.</p>
          ...
          {!isLoading && !error && histories.length > 0 && (
            <div className="inline-flex items-center gap-8 mt-7 px-8 py-4 bg-white border border-gray-100 rounded-2xl shadow-sm">
              <div className="text-center"><p className="text-2xl font-extrabold text-gray-800">{histories.length}</p><p className="text-xs text-gray-400 font-semibold mt-0.5">Total Riwayat</p></div>
              <div className="w-px h-10 bg-gray-100"></div>
              <div className="text-center"><p className="text-2xl font-extrabold text-primary-50">{totalHerbsAll}</p><p className="text-xs text-gray-400 font-semibold mt-0.5">Total Herbal</p></div>
              <div className="w-px h-10 bg-gray-100"></div>
              <div className="text-center"><p className="text-2xl font-extrabold text-amber-500">{totalFavorites}</p><p className="text-xs text-gray-400 font-semibold mt-0.5">⭐ Favorit</p></div>
            </div>
          )}
        </div>
          ...
          <div className="bg-white/80 backdrop-blur-xl border border-gray-100 rounded-2xl p-4 shadow-sm mb-10 flex flex-col sm:flex-row gap-3 animate-[slideDown_0.4s_ease-out]">
            <div className="relative flex-1">
              <div className="absolute inset-y-0 left-3.5 flex items-center pointer-events-none text-gray-400">
                <svg className="h-4 w-4" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor"><path fillRule="evenodd" d="M8 4a4 4 0 100 8 4 4 0 000-8zM2 8a6 6 0 1110.89 3.476l4.817 4.817a1 1 0 01-1.414 1.414l-4.816-4.816A6 6 0 012 8z" clipRule="evenodd" /></svg>
              </div>
              <input type="text" placeholder="Cari penyakit atau gejala..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)}
            ...
            <div className="flex gap-2 items-center">
              <div className="flex bg-gray-50 p-1 rounded-xl border border-gray-100 gap-1">
                {[{ id: "semua", label: "Semua" }, { id: "minggu", label: "Minggu Ini" }, { id: "bulan", label: "Bulan Ini" }, { id: "favorit", label: "⭐ Favorit" }].map((btn) => (
                  <button key={btn.id} onClick={() => setTimeFilter(btn.id)}
                    className={`flex-1 px-3 py-2 text-xs font-bold rounded-lg transition-all duration-200 whitespace-nowrap
                      ${timeFilter === btn.id
                        ? btn.id === "favorit" ? "bg-amber-50 text-amber-600 shadow-sm border border-amber-100" : "bg-white text-primary-50 shadow-sm border border-gray-100"
              ...
              {!isSelectMode
                ? <button onClick={enterSelectMode} className="shrink-0 inline-flex items-center gap-1.5 px-3.5 py-2.5 bg-gray-50 hover:bg-red-50 border border-gray-100 hover:border-red-200 text-gray-500 hover:text-red-500 rounded-xl text-xs font-bold transition-all duration-200">
                ...
                : <button onClick={exitSelectMode} className="shrink-0 px-3.5 py-2.5 bg-gray-100 hover:bg-gray-200 border border-gray-200 text-gray-600 rounded-xl text-xs font-bold transition-all duration-200">Batal</button>
              }
            </div>
          </div>
        )}
        ...
        {!isLoading && error && (
          <div className="bg-white border border-red-100 rounded-[28px] p-12 text-center shadow-sm">
            ...
            <button onClick={fetchHistory} className="px-6 py-2.5 bg-primary-50 text-white rounded-full text-sm font-bold hover:bg-primary-60 transition-colors">Coba Lagi</button>
          </div>
        )}
        ...
        {!isLoading && !error && histories.length === 0 && (
          <div className="bg-white border-2 border-dashed border-gray-200 hover:border-primary-20 transition-colors rounded-[28px] p-16 text-center">
            <div className="w-20 h-20 bg-gray-50 rounded-full flex items-center justify-center mx-auto mb-5"><span className="text-4xl">🌿</span></div>
            <h3 className="text-xl font-extrabold text-gray-700 mb-2">Belum Ada Riwayat</h3>
            <p className="text-gray-400 text-sm mb-8 max-w-xs mx-auto leading-relaxed">Mulai analisis herbal pertama Anda dan hasilnya akan tersimpan otomatis di sini.</p>
        ...
        {!isLoading && !error && histories.length > 0 && isFavoritTab && filteredHistories.length === 0 && (
          <div className="bg-white border-2 border-dashed border-amber-100 hover:border-amber-200 transition-colors rounded-[28px] p-16 text-center">
            ...
            <p className="text-gray-400 text-sm max-w-xs mx-auto leading-relaxed">Tandai riwayat pencarian dengan ⭐ untuk menyimpannya di sini.</p>
          </div>
        )}
        ...
        {!isLoading && !error && histories.length > 0 && !isFavoritTab && filteredHistories.length === 0 && (
          <div className="bg-white border border-gray-100 rounded-[28px] p-12 text-center shadow-sm">
            ...
            <h4 className="text-base font-bold text-gray-700 mb-2">Tidak Ditemukan</h4>
            <p className="text-gray-400 text-sm mb-5">Tidak ada riwayat yang cocok dengan pencarian Anda.</p>
            <button onClick={() => { setSearchTerm(""); setTimeFilter("semua"); }} className="px-5 py-2.5 bg-gray-100 hover:bg-gray-200 text-gray-600 rounded-full text-sm font-bold transition-colors">Reset Filter</button>
          </div>
        )}
        ...
        {!isLoading && !error && filteredHistories.length > 0 && (
          <div className="space-y-4">
            {filteredHistories.map((hist, index) => (
              <RiwayatCard key={hist.id} hist={hist} index={index} onDelete={setDeleteTarget}
                onSelectHerb={(herb, chemical) => { setSelectedHerb(herb); setSelectedHerbChemical(!!chemical); }}
                isFavorite={favorites.has(hist.id)} onToggleFavorite={handleToggleFavorite}
                isSelectMode={isSelectMode} isSelected={selectedIds.has(hist.id)} onToggleSelect={handleToggleSelect} />
            ))}
          </div>
        )}
      </div>
      ...
      {isSelectMode && (
        <div className="fixed bottom-6 inset-x-0 flex justify-center px-4 z-40 pointer-events-none animate-[slideUp_0.3s_ease-out]">
          <div className="pointer-events-auto bg-white border border-gray-200 rounded-[28px] shadow-2xl shadow-black/10 px-4 py-3 flex items-center gap-3 max-w-lg w-full">
            <button
              onClick={isAllSelected ? () => setSelectedIds(new Set()) : () => setSelectedIds(new Set(filteredHistories.map(h => h.id)))}
            ...
            >
              <div className={`w-4 h-4 rounded border-2 flex items-center justify-center transition-all ${isAllSelected ? "bg-primary-50 border-primary-50" : "border-gray-300"}`}>
                {isAllSelected && <svg xmlns="http://www.w3.org/2000/svg" className="h-2.5 w-2.5 text-white" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={3}><path strokeLinecap="round" strokeLinejoin="round" d="M5 13l4 4L19 7" /></svg>}
              </div>
              {isAllSelected ? "Batal Semua" : "Pilih Semua"}
            </button>
            <div className="flex-1 text-center">
              {selectedIds.size === 0
                ? <p className="text-xs text-gray-400 font-semibold">Belum ada yang dipilih</p>
                : <p className="text-sm font-extrabold text-gray-800"><span className="text-red-500">{selectedIds.size}</span> dipilih</p>}
            </div>
            <button
              onClick={() => { if (selectedIds.size > 0) setShowMultiDeleteModal(true); }}
      ...
      {selectedHerb && <HerbDetailModal herb={selectedHerb} onClose={() => { setSelectedHerb(null); setSelectedHerbChemical(false); }} showMedicalWarning={selectedHerbChemical} />}
