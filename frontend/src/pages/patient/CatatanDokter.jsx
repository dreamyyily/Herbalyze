...
// Contract Address yang sudah di-deploy ke Ganache
const CONTRACT_ADDRESS = "0x2c2b9927Ff7f00B9eA88f141Bb5fCD4784Ef4C23";
const CONTRACT_ABI = [
...
export default function CatatanDokter() {
  ...
  const getProvider = () => {
    // Gunakan RPC Ganache langsung (read-only, tidak perlu MetaMask untuk baca)
  ...
  const fetchRecordsFromBlockchain = async () => {
    ...
    try {
      ...
      // Ambil total jumlah record yang ada di blockchain
      ...
      // Loop semua record, filter yang milik wallet pasien ini
      for (let i = 1; i <= total; i++) {
        try {
          const [encryptedData, patientAddress, uploader, timestamp] = await contract.getMedicalRecord(i);

          // Hanya ambil record milik wallet user yang sedang login
          // Dokter melihat semua yang dia upload, Pasien hanya melihat miliknya
          const isPatientRecord = patientAddress.toLowerCase() === userWallet;
          const isDoctorRecord = role === 'Doctor' && uploader.toLowerCase() === userWallet;

          if (isPatientRecord || isDoctorRecord) {
            // Tentukan kunci dekripsi: gunakan patientAddress sebagai kunci
            ...
            try {
              ...
              // Jika gagal dekripsi (mungkin bukan catatan untuk wallet ini)
              console.warn(`Record #${i} tidak bisa didekripsi, mungkin bukan milik akun ini.`);
            }
          }
        } catch (recordErr) {
  ...
  // ─────────────────────────────────────────────────────────
  // SIMPAN CATATAN BARU KE BLOCKCHAIN (HANYA DOKTER)
  // ─────────────────────────────────────────────────────────
  ...
  const handleSimpanKeBlockchain = async () => {
    ...
    try {
      ...
      // 2. Enkripsi dengan wallet pasien sebagai kunci
      ...
      // 4. Kirim ke Smart Contract
      const contract = new ethers.Contract(CONTRACT_ADDRESS, CONTRACT_ABI, signer);
      console.log("Mengirim data terenkripsi ke blockchain...");
      ...
      setPatientWallet("");
      fetchRecordsFromBlockchain(); // Refresh tabel setelah simpan
              ...
              {isFetching ? (
                ...
                <div className="flex flex-col items-center justify-center py-20 bg-gray-50 rounded-2xl border border-dashed border-gray-300">
                  ...
                  <p className="text-gray-400 text-sm mt-1">
                    {role === 'Doctor' ? 'Tambahkan catatan baru untuk pasien' : 'Dokter belum menambahkan catatan untuk Anda'}
                  </p>
                </div>
              ) : (
