...
export default function RekamMedis() {
  ...
  useEffect(() => {
    ...
    fetch(`http://127.0.0.1:8000/api/profile/${wallet}`)
      ...
      .then((data) => {
        setProfile({
          name: data.name,
          instansi: data.instansi || "Rumah Sakit",
        });
      })
    ...
    const interval = setInterval(async () => {
      ...
      try {
        ...
        const uniquePatients = [
          ...new Set(patients.map((p) => p.toLowerCase())),
        ];
  ...
  const refreshBlockchainSilent = async () => {
    try {
      ...
      for (let i = total; i >= Math.max(1, total - 50); i--) {
        try {
          const [encryptedData, patientAddress, , timestamp] =
      ...
      const patientWallets = [
        ...new Set(
          myRecords.map((r) => r.patientAddress?.toLowerCase()).filter(Boolean),
        ),
      ];
      if (patientWallets.length > 0) {
        await fetchPatientProfiles(patientWallets);
      }
      ...
      setRecords((prev) => {
        ...
        if (newOnes.length > 0) {
          ...
          showToast(
            "info",
            "Rekam Medis Baru",
            `${newOnes.length} rekam medis baru berhasil disimpan ke Blockchain oleh pasien!`,
          );
  ...
  const fetchPatientProfiles = async (walletAddresses) => {
    try {
      const response = await fetch(
        "http://localhost:8000/api/patients/by-wallets",
        {
          method: "POST",
  ...
  const handleSubmitDraft = async () => {
    if (
      ...
      !formData.gejala ||
      !formData.obat ||
      !formData.kondisiKhusus
    ) {
      showToast(
        "error",
        "Validasi Gagal",
        "Harap lengkapi semua field yang bertanda bintang (*)",
      );
      return;
    }
    if (!patientWallet.startsWith("0x") || patientWallet.length !== 42) {
      showToast(
        "error",
        "Format Tidak Valid",
        "Format Wallet Address Pasien tidak valid!",
      );
      return;
    }
    ...
    if (!isConsented) {
      showToast(
        "error",
        "Izin Ditolak",
        "Pasien belum memberikan izin kepada Anda. Rekam medis tidak bisa ditambahkan.",
      );
      return;
    }
    ...
    try {
      ...
      const res = await fetch(
        "http://localhost:8000/api/medical-record/draft",
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
      ...
      if (!res.ok) {
        showToast(
          "error",
          "Gagal Submit",
          data.error || data.detail || "Terjadi kesalahan saat memproses data.",
        );
        return;
      }
      ...
      showToast(
        "success",
        "Draft Tersimpan!",
        "Rekam medis berhasil disimpan. Pasien akan mendapat notifikasi untuk memverifikasi data ini sebelum masuk ke Blockchain.",
      );
      setIsFormOpen(false);
      setFormData({
        diagnosis: "",
      ...
      showToast(
        "error",
        "Server Error",
        "Gagal menyimpan draft rekam medis. Cek koneksi server.",
      );
    } finally {
      setIsLoading(false);
    }
  ...
  const filteredRecords = records.filter(
    (r) =>
      ...
      (patientProfiles[r.patientAddress?.toLowerCase()]?.name || "")
        .toLowerCase()
  ...
  // Pagination for consentedPatients
  const patientsTotalPages = Math.ceil(
    consentedPatients.length / ITEMS_PER_PAGE,
  );
  const patientsStartIndex = (currentPagePatients - 1) * ITEMS_PER_PAGE;
  const patientsEndIndex = patientsStartIndex + ITEMS_PER_PAGE;
  ...
  const recordsStartIndex = (currentPageRecords - 1) * ITEMS_PER_PAGE;
  const recordsEndIndex = recordsStartIndex + ITEMS_PER_PAGE;
        ...
        {!isFormOpen && !selectedRecord && (
          <div className="text-center mb-10">
            ...
            <p className="text-gray-500">
              Kelola rekam medis pasien yang telah memberikan izin kepada Anda
            </p>
          </div>
        )}
            ...
            <div className="animate-fade-in">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div className="bg-gray-50 rounded-xl p-5 border border-gray-100">
                  ...
                  <p className="font-semibold text-gray-800">
                    {patientProfiles[
                      selectedRecord.patientAddress?.toLowerCase()
                    ]?.name ||
                      (selectedRecord.patientAddress
                        ? `${selectedRecord.patientAddress.substring(0, 6)}...${selectedRecord.patientAddress.slice(-4)}`
                        : "-")}
                  </p>
                </div>
                <div className="bg-gray-50 rounded-xl p-5 border border-gray-100">
            ...
            <div className="animate-fade-in">
              ...
              <div className="mt-8 flex justify-end">
                ...
                >
                  {isLoading
                    ? "⏳ Menyimpan Draft..."
                    : "📋 Kirim ke Pasien untuk Diverifikasi"}
                </button>
              </div>
            </div>
          )}
            ...
            <>
                ...
                <div>
                  <div className="flex justify-between items-center mb-6">
                    <p className="text-sm text-gray-500">
                      Pasien yang telah memberikan izin kepada Anda
                    </p>
                    <button
                  ...
                  {isFetchingPatients ? (
                    ...
                    <div className="flex flex-col items-center justify-center py-20 bg-gray-50 rounded-2xl border border-dashed border-gray-300">
                      ...
                      <p className="text-gray-500 font-medium">
                        Belum ada pasien yang memberi izin
                      </p>
                    ...
                    <>
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        {paginatedPatients.map((wallet) => {
                          const norm = wallet.toLowerCase();
                          const p = patientProfiles[norm];
                          return (
                            <div
                              key={wallet}
                              className="border border-green-100 bg-green-50 rounded-2xl p-6"
                            >
                      ...
                      {patientsTotalPages > 1 && (
                        <div className="mt-8 flex flex-col sm:flex-row items-center justify-between gap-4 bg-white border border-gray-100 rounded-2xl p-4 shadow-sm">
                          <button
                            onClick={() =>
                              setCurrentPagePatients(
                                Math.max(1, currentPagePatients - 1),
                              )
                            }
                            ...
                            className={`inline-flex items-center gap-2 px-4 py-2 rounded-xl font-semibold text-sm transition-all ${
                              currentPagePatients === 1
                                ...
                                : "bg-gray-50 text-gray-600 hover:bg-green-10 hover:text-green-50"
                            }`}
                          >
                            <svg
                              xmlns="http://www.w3.org/2000/svg"
                          ...
                          <button
                            ...
                            disabled={
                              currentPagePatients === patientsTotalPages
                            }
                            className={`inline-flex items-center gap-2 px-4 py-2 rounded-xl font-semibold text-sm transition-all ${
                              currentPagePatients === patientsTotalPages
                                ? "bg-gray-50 text-gray-300 cursor-not-allowed"
              ...
              {activeTab === "rekam" && (
                <div>
                  <div className="flex justify-between items-center mb-6">
                    <p className="text-sm text-gray-500">
                      Semua rekam medis yang telah Anda tambahkan
                    </p>
                  ...
                  {isFetching ? (
                    ...
                    <div className="flex flex-col items-center justify-center py-20 bg-gray-50 rounded-2xl border border-dashed border-gray-300">
                      ...
                      <p className="text-gray-400 text-sm mt-1">
                        Rekam medis akan muncul di sini setelah pasien
                        menyetujui
                      </p>
                    </div>
                  ) : (
                        ...
                        <tbody>
                          {paginatedRecords.map((record) => (
                            ...
                            >
                              ...
                              <td className="py-4 px-4 text-gray-700 font-medium">
                                {patientProfiles[
                                  ...
                                  (record.patientAddress
                                    ? record.patientAddress.substring(0, 6) +
                                      "..." +
                              ...
                              <td className="py-4 px-4 text-gray-600 max-w-[150px] truncate">
                                {record.obat || "-"}
                              </td>
                      ...
                      {recordsTotalPages > 1 && (
                        <div className="flex flex-col sm:flex-row items-center justify-between gap-4 bg-gray-50 border-t border-gray-100 p-4">
                          <button
                            onClick={() =>
                              setCurrentPageRecords(
                                Math.max(1, currentPageRecords - 1),
                              )
                            }
                          ...
                          <button
                            ...
                            disabled={
                              currentPageRecords === recordsTotalPages
                            }
                            className={`inline-flex items-center gap-2 px-4 py-2 rounded-xl font-semibold text-sm transition-all ${
                              currentPageRecords === recordsTotalPages
                                ? "bg-white text-gray-300 cursor-not-allowed border border-gray-200"
